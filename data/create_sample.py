"""Run this once to generate sample_employees.csv for the example config."""
import numpy as np
import pandas as pd
...
years_float = years_experience.astype(float)
years_float[missing_idx] = np.nan
...
print(f"Created {out} with {len(df)} rows, {df.isna().sum().sum()} missing values")
