"""
report_logic.py — All data-loading, plotting, and model-fitting functions.

Import from here to reuse pieces without running the full pipeline:
    from report_logic import load_config, load_data, fit_model, plot_continuous
"""
import base64
import io
import warnings
...
def get_variable_groups(config: dict):
    """Return (dependent_vars, independent_vars) from config."""
...
def fit_model(df: pd.DataFrame, config: dict,
    ...
    """Route to the correct model fitter based on config['model']['type']."""
